...
switch ($action) {
    ...
    case 'addwecker':
        ...
        if ($weekdays != '' && $playlist != '' && $hour != '' && $minute != '' && $onoff != '' && $duration != '' && $fade != '' && $dimm != '' && $start_volume != '' && $end_volume != '') {
            ...
            addElement($xmldoc, $xml_root, $weekdays, $hour, $minute, $duration, $playlist, $onoff, $fade, $dimm, $power_supplies_on, $power_supplies_off, $start_volume, $end_volume, $tts);
            $fp = fopen($file, "w");
...
function addElement(&$xmldoc, &$xml_root, $weekdays, $hour, $minute, $duration, $playlist, $onoff, $fade, $dimm, $power_supplies_on, $power_supplies_off, $start_volume, $end_volume, $tts) {
    ...
    // Dimm
    $xml_dimm = $xmldoc->createElement("dimm", $dimm);
    $xml_wecker->appendChild($xml_dimm);
...
function getWecker($shortweekday = true) {
    $file = "/var/www/wecker/alarms.xml";
    ...
    foreach ($xml_root as $wecker) {
        ...
        $al['dimm'] = $wecker->getElementsByTagName("dimm")->item(0)->nodeValue;
    ...
    aasort($alarms, "uhrzeit");
    return $alarms;
}

function getWeckerfromID($ID) {
    $alarms = getWecker(false);
