    ...
    $(document).ready(function() {
        ...
        $("#slider").slider({
            ...
            slide: function(event, ui) {
                ...
                $('#slider_content').html(value);
            },
            change: function(event, ui) {
                var value = $(this).slider("option", "value");
                $('#slider_content').html(value);
                $( "#slider" ).slider( "option", "disabled", true );
                $.get('./lib/motor.php?action=dimm&value='+value, function(){
        ...
        $.ajax({
            url: './lib/motor.php?action=getlaststatus',
            success: function(data) {
                 $("#slider").slider("option", "value", data );
                 $('#slider_content').html(data);
            }
        });
        $('.button_on.dimmer').click(function() {
            if (!$( "#slider" ).slider( "option", "disabled" ))
                $("#slider").slider("option", "value", 1024 );
        });
        $('.button_off.dimmer').click(function() {
            if (!$( "#slider" ).slider( "option", "disabled" ))
        ...
        $('.display').click(function() {
            ...
            if (value == 'on'){
                $.get('lib/display.php?action=on');
            }else if(value == 'off'){
                $.get('lib/display.php?action=off');
            }else if (value != '' ){ 
                $.get('lib/display.php?action=dimm&value=' + value);
            }
        });
    });
</script>
