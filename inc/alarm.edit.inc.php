<div class="panel panel-default">
    ...
    <?php } ?>
                    ...
                    $("#addform").submit(function(e) {
                        ...
                        $.get('./lib/weckerpy.php?action=editwecker&id=' + <?= $id ?> + '&weekday=' + weekday + '&hour=' + hour + '&minute=' + minute + '&playlist=' + playlist + '&fade=' + fade + '&dimm=' + dimm + '&duration=' + duration + '&onoff=' + onoff + '&power-on=' + power_on + '&power-off=' + power_off + '&start-volume=' + start_volume + '&end-volume=' + end_volume + '&tts=' + tts, function(data) {
                            if (data != 'True') {
                                alert(data);
                            }
            ...
            <form role="form" id="addform">
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" id="wecker_duration" type="number" size="1" placeholder="Duration" value="<?= $alarms['Dauer'] ?>" class="nomargb">
                    </div>

                    <div class="form-group">
                        <label class="control-label" for="wecker_start_volume">Start Volume</label>
                        <input class="form-control" id="wecker_start_volume" type="number" size="1" value="<?= $alarms['startVol'] ?>" value="30" class="nomargb">
                    </div>
                    <div class="checkbox" id="wecker_fade">
                        <label for="wecker_fadestatus">
                            <input type="checkbox" class="form-control" id="wecker_fadestatus"  <?= $alarms['Fade'] == 1 ? 'checked' : '' ?> data-toggle="checkbox">Fade Music In</label>
                    </div>
                    <div id="fade_div">
                        <div class="form-group">
                            <label class="control-label" for="wecker_end_volume">End Volume</label>
                            <input class="form-control" id="wecker_end_volume" type="number" size="1"  value="<?= $alarms['endVol'] ?>" class="nomargb">
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="wecker_power_on" class="select-block" multiple>
                            ...
                            foreach ($sockets as $socket) {
                                if (in_array($socket, $alarms['power-on']))
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="wecker_power_off" class="select-block" multiple>
                            ...
                            $sockets = getSockets($config);
                            foreach ($sockets as $socket) {
                                if (in_array($socket, $alarms['power-off'])) {
                                    echo '<option value="' . urlencode($socket) . '" selected>' . $socket . '</option>';
                                } else {
                                    echo '<option value="' . urlencode($socket) . '">' . $socket . '</option>';
                                }
                            }
                    ...
                    <div class="checkbox" id="wecker_dimm" >
                        <label for="wecker_dimmstatus">
                            <input type="checkbox" class="form-control" value="" id="wecker_dimmstatus" <?= $alarms['dimm'] == 1 ? 'checked' : '' ?> data-toggle="checkbox">Dimmer</label>
                    </div>
                    <div class="checkbox" id="wecker_onoff" >
                        <label for="wecker_status">
                            <input type="checkbox" class="form-control" value="" id="wecker_status" <?= $alarms['Active'] == 'ON' ? 'checked' : '' ?> data-toggle="checkbox">Active</label>
                    </div>
                    <div class="form-group">
                        <label class="control-label" for="wecker_tts">TTS</label>
                        <textarea class="form-control" id="wecker_tts" rows="3" placeholder="Text to say"><?= $alarms['tts'] ?></textarea>
                    ...
                    <button type="submit" class="btn btn-primary">Edit Alarm</button>
